...
resource "datadog_monitor" "tfer--monitor_103959101" {
  ...
  priority             = "0"
  query                = "sum(last_10m): ( sum:trace.flask.request.errors{env:development,service:discountsservice}.as_count() / sum:trace.flask.request.hits{env:development,service:discountsservice}.as_count() ) > 0.05"
  renotify_interval    = "0"
  renotify_occurrences = "0"
...
resource "datadog_monitor" "tfer--monitor_103959103" {
  ...
  notify_no_data       = "false"
  priority             = "0"
  query                = "avg(last_10m): ( sum:trace.flask.request.duration{env:development,service:discountsservice}.rollup(sum).fill(zero) / sum:trace.flask.request.hits{env:development,service:discountsservice} ) > 0.5"
  renotify_interval    = "0"
  renotify_occurrences = "0"
  require_full_window  = "false"
  tags                 = ["env:development", "service:discountsservice"]
...
resource "datadog_monitor" "tfer--monitor_103959107" {
  ...
  notify_no_data       = "false"
  priority             = "0"
  query                = "sum(last_10m): ( sum:trace.rack.request.errors{env:development,service:store-frontend}.as_count() / sum:trace.rack.request.hits{env:development,service:store-frontend}.as_count() ) > 0.05"
  renotify_interval    = "0"
  renotify_occurrences = "0"
  require_full_window  = "false"
...
resource "datadog_monitor" "tfer--monitor_103959108" {
  ...
  notify_no_data       = "false"
  priority             = "0"
  query                = "avg(last_10m):( sum:trace.rack.request.duration{env:development,service:store-frontend}.rollup(sum).fill(zero) / sum:trace.rack.request.hits{env:development,service:store-frontend} ) > 0.4"
  renotify_interval    = "0"
  renotify_occurrences = "0"
  require_full_window  = "false"
  tags                 = ["env:development", "service:store-frontend"]
...
resource "datadog_monitor" "tfer--monitor_103959109" {
  ...
  notify_no_data       = "false"
  priority             = "0"
  query                = "avg(last_10m): ( sum:trace.postgres.query.duration{env:development,service:postgres}.rollup(sum).fill(zero) / sum:trace.postgres.query.hits{env:development,service:postgres} ) > 0.5"
  renotify_interval    = "0"
  renotify_occurrences = "0"
  require_full_window  = "false"
